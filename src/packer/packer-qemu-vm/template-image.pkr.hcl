...
variable "iso_url" {
  type    = string
  default = "http://cloud-images.ubuntu.com/releases/jammy/release/ubuntu-22.04-server-cloudimg-amd64.img"
}
variable "iso_checksum" {
  type    = string
  default = "http://cloud-images.ubuntu.com/releases/jammy/release/SHA256SUMS"
}
...
locals {
  ...
  version    = chomp(file("${path.cwd}/version"))
  image_name = "${var.vm_name}-${var.release}-${local.version}"
  output_dir = "${var.output_dir}/${local.image_name}"
}
...
#    hcp_packer_registry {
 #     bucket_name = "pyrobuntu2204"
  #    description = "Personal Ubuntu 22.04 Build for Kubernetes VMs"
   #   bucket_labels = {
#        "owner"          = "pyromaniac"
 #       "os"             = "Ubuntu",
...
#   provisioner "shell" {
#     environment_vars = [
#       "INSTALL_DEV_PACKAGES=${var.install_dev_packages}"
#     ]
#    execute_command   = "echo 'pyromaniac' | {{.Vars}} sudo -S -E bash -eux '{{.Path}}'"
...
#  }
  ...
  #	inline = [
  #		echo "local version ${local.version}",
  #	]
  #}
  ...
  post-processor "shell-local" {
    keep_input_artifact = true
    inline = [
      "mv ${local.output_dir}/${local.image_name} ${local.output_dir}/${local.image_name}.qcow2",
    ]
  }
